...
const Stage={cache:{},
  build(themeId){
    ...
    // hanging chains, kept within the framed area around the arch
    const chainXs=[cx-330,cx-190,cx+190,cx+330,cx-70,cx+70];
    for(let ci=0;ci<chainXs.length;ci++){const cxs=chainXs[ci],len=80+r.int(60);
      wc.strokeStyle='#12141c';wc.lineWidth=3;
      for(let y=0;y<len;y+=14){wc.beginPath();wc.ellipse(cxs+((y/14)%2?3:-3),y,5,7,0,0,Math.PI*2);wc.stroke()}
      wc.fillStyle='#1c2030';wc.beginPath();wc.arc(cxs,len,7,0,Math.PI*2);wc.fill()}
  ...
  draw(c,cam,frame,st){
    ...
    for(let i=0;i<st.torches.length;i++){const t=st.torches[i],flick=((frame*7+i*13)%17)/17,rad=52+12*flick;
      ...
      c.fillStyle=`rgba(255,${210+r_flame(flick)},150,.95)`;c.beginPath();c.ellipse(t.x,t.y-8-flick*3,4,9+flick*3,0,0,Math.PI*2);c.fill()}}};
function r_flame(flick){return Math.floor(20*flick)}
const Camera={
