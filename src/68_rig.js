...
const Rig={
  ...
  _drawBrute(c,F,look,j,face,scale,poseKey,lit){
    ...
    const leg=(hp,kn,ft)=>{
      ...
      BodyStyle.seam(c,kn.x,kn.y,Math.min(thighW*.76,shinW)*.5-1.4,look,face,jc);
    ...
    for(const p of look.props||[]){
      ...
      if(p==='spikedclub'){ // a bigger, spiked take on the human rig's 'club' prop
        ...
        c.strokeStyle=look.body.outline;c.lineWidth=19;c.lineCap='round';
        c.beginPath();c.moveTo(h.x,h.y);c.lineTo(h.x+face*len*.3,h.y-len);c.stroke();
        const sg=c.createLinearGradient(h.x-8,h.y,h.x+10,h.y-len);
        sg.addColorStop(0,shade(look.secondary,-.32));sg.addColorStop(.4,shade(look.secondary,.14));
        sg.addColorStop(1,shade(look.secondary,-.26));
        c.strokeStyle=sg;c.lineWidth=16;
        c.beginPath();c.moveTo(h.x,h.y);c.lineTo(h.x+face*len*.3,h.y-len);c.stroke();
        for(let i=0;i<4;i++){const t=.25+i*.2,px=h.x+face*len*.3*t,py=h.y-len*t;
          c.fillStyle=look.body.outline;
          c.beginPath();c.moveTo(px,py-1);c.lineTo(px+face*9,py-4);c.lineTo(px,py+6.5);c.closePath();c.fill();
          c.fillStyle='#c9c2b0';
