...
// Fix round 1 (controller review): the first pass tuned blU/blL down (76/14 -> 60/0) purely to keep
// the strike paw's own y from disappearing under the body capsule's painted footprint, since drawQuad
// used to draw the hind legs BEFORE that capsule (i.e. behind/under it) regardless of pose — the kick
// passed the numeric pose test with room to spare but was nearly invisible in a real screenshot.
// drawQuad now draws the striking hind legs AFTER the capsule during this pose specifically (see its
// own comment), which removes the occlusion problem entirely — so the angles are back to the original,
// more dramatic 76/14 (blU/blL) for a clearer, further-reaching kick silhouette now that nothing hides
// it. The strike keyframe's off.y:6 (new) is the "slight hip drop" the review asked for: it lowers the
// whole rear during the kick, which costs nothing in reach (Rig.extent's reach formula excludes off.y
// entirely, see its own comment) and keeps the front paws planted almost exactly at their idle y
// (verified: -1.0 vs idle's own -3.6 for donut) despite the hip itself sitting lower.
POSES_QUAD.medium=[
  ...
  {t:.43,ang:{spine:D(2),chest:D(2),neck:D(28),tail1:D(18),tail2:D(-8),
             ...
             blU:D(76),blL:D(14),brU:D(70),brL:D(18)},off:{x:6,y:6}},
...
const Rig={
  ...
  drawQuad(c,F,cam,frame,look){
    ...
    const skinDark=shade(look.skin,-.35);
    // Fix round 1 (controller review, Task 6.5): the medium kick's hind legs used to be drawn in the
    // same fixed spot as every other pose -- BEHIND the body capsule below, since that's simply the
    // order this function always drew in -- so no pose data could ever make the kick clear the body's
    // own paint; it read as invisible in a real screenshot despite passing the numeric pose test with
    // room to spare. `kicking` gates a second, later draw of the hind legs (after the body/front legs,
    // see below) specifically for this one pose, instead of reordering the whole function for every
    // pose (which would put the hind legs in front of the body during idle/walk/light1/etc too, where
    // nothing asked for that and it would read as a rendering regression, not a fix).
    const kicking=key==='medium';
    c.save();c.translate(F.x,FLOOR);c.scale(scale,scale);
    ...
    // pad (Fix round 1): an optional second arg draws a bigger, tinted "kick paw" -- a visible paw pad
    // (a smaller lighter oval set into the fill, the same "shade a highlight into a flat fill" trick
    // drawBig's own scars/brow-ridge props already use) instead of the ordinary flat ellipse, so the
    // striking paw itself reads as a deliberate foot, not just a leg-end. col overrides the fill/
    // stroke entirely (the kicking limbs' own darker tint, see below); undefined keeps the ordinary look.
    ...
    // back legs first (behind the body), then the tail (base tucks under the hip), UNLESS this is the
    // kick (see `kicking` above) -- then they're skipped here and drawn again, in front, after the
    // body/front legs below. Upper legs noticeably thicker than lower (.55 taper, was .78 in round 1 —
    // art review called the whole rig "wire-bodied" and this taper is part of giving it real limb
    // volume, matching the human rig's own thigh/shin taper).
    if(!kicking){
    ...
    // Fix round 1 (controller review, Task 6.5): the kick's hind legs, drawn here instead of their
    // usual spot above (see `kicking`'s own comment) so the body capsule and front legs can never hide
    // them -- a shade darker than the body fill (kickCol) so the striking limb visually separates from
    // the body even where it overlaps it, plus the bigger, pad-marked paw() variant, both per the
    // review's "make the strike more legible" ask.
    if(kicking){
      const kickCol=shade(look.skin,-.22);
      limb(j.blHip,j.bl1,look.legW,kickCol);limb(j.bl1,j.bl2,look.legW*.55,kickCol);paw(j.bl2,kickCol,true);
      limb(j.brHip,j.br1,look.legW,kickCol);limb(j.br1,j.br2,look.legW*.55,kickCol);paw(j.br2,kickCol,true)}
    for(const p of look.props||[]){
