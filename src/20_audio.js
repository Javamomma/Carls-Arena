const Audio={ac:null,muted:Save.data.mute,_t:0,
  ...
  recipes:{
    ...
    // Fix-wave item 5 (final review M1): Task 7.4 routed moveName==='light' through G.nodeRecipe
    // (80_game.js), which only ever returns light1..light5 -- the 'light' entry that used to live here
    // (added in Task 7.2 for a lookup Task 7.4 later replaced) is unreachable: the only other
    // Audio.recipes[...] lookup is checkSpecial's, gated to s1/s2. Deleted rather than kept as a
    // fallback nothing calls. light1..light5 stay -- still used as fixed sub-hit/testing tones
    // (80_game.js's multi-hit special sub-thud always plays light1) and by G.nodeRecipe itself.
