...
Tasks: 2.1 camera + stage; 2.2 sprite factory + pose mapping (test: every champ has every pose, screenshots of two poses differ by >2% pixels); 2.3 hit feel (hitstop per move, shake, sparks, damage popups, KO slow-mo); 2.4 sfx recipes per move + announcer port; 2.5 crit/armor/block proficiency (tests with `noCrit` and forced rng); 2.6 combo tuning (medium ender push-out, wall bounce, parry window measured against a human via `--probe 'G.fight.p2.blockAge'` histogram) ; a failed parry attempt must cost something (short block lockout before re-arm) because mash-blocking 6-on/1-off currently out-parries holding block 7:1 (final review of Phase 1); 2.7 champions Mongo (tank) and Katia (trickster) with unique heavies/specials; 2.8 S3 cinematic (freeze, zoom, name card, unskippable 1.2 s).
