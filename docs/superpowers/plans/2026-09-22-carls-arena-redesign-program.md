...
| 8 — Art upgrade | Richer procedural vector bodies (layered shapes, gradients, outlines, faces, cloth) for all eleven looks; stage depth and lighting; HUD chevrons and portrait frames; door and roster art. Covers roadmap item 7 ("ships 'not stick figures any more' with no assets"). | **DONE** (Tasks 8.0-8.5, commits `5bc5397..8022f2e` on `main`) | [`2026-09-22-phase8-art-upgrade.md`](2026-09-22-phase8-art-upgrade.md) | Every look's new rig passes the existing HUD-clearance/reach/pose tests unchanged; `tools/shots.sh`'s full set regenerated and reviewed (58 PNGs); no new asset files required (still zero-asset by default) — see `docs/ARENA.md`'s "Phase 8 exit" section for the full gate, the perf trend (0.124→0.283ms, well under the 6ms budget throughout), and the rulings copied from the SDD ledger |
